...
@section('content')
    <div class="col-md-10 p-4" style="background: #f4f5f7">
        <h3 class="mb-4">✏️ Edit KPIs for: <strong>{{ $position->title }}</strong></h3>
        ...
        <form action="#" method="POST">
            ...
            <table class="table table-bordered" id="kpi-table">
                ...
                @foreach ($kpis as $index => $kpi)
                    <tr>
                        ...
                        <td><input name="kpis[{{ $index }}][name]" value="{{ $kpi->name }}" class="form-control" required></td>
                        <td><input name="kpis[{{ $index }}][how_to_measure]" value="{{ $kpi->how_to_measure }}" class="form-control" required></td>
                        <td><input name="kpis[{{ $index }}][target]" value="{{ $kpi->target }}" class="form-control" required></td>
                        <td><input type="number" name="kpis[{{ $index }}][weight]" value="{{ $kpi->weight }}" class="form-control weight-field" required></td>
            ...
            <button type="submit" class="btn btn-success">💾 Update KPIs</button>
        </form>
    </div>

    <script>
        let rowIndex = {{ count($kpis) }};
        ...
        document.getElementById('add-row').addEventListener('click', function () {
            ...
            row.querySelectorAll('input').forEach((input, i) => {
                ...
                if (name.includes('[id]')) {
                    input.remove(); // don't clone hidden ID field for new entries
                } else {
                    input.value = '';
                }
            });
